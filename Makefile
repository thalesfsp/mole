...
LDFLAGS := -X github.com/thalesfsp/mole/cmd.version=$(version)

test:
ifneq ($(shell go fmt ./...),)
	$(error code not formatted. Please run 'go fmt')
endif
	@go test github.com/thalesfsp/mole/... -v -race -coverprofile=coverage.txt -covermode=atomic
cover: test
...
build:
	@go build -o bin/mole github.com/thalesfsp/mole
install:
...
ifeq ($(version),)
	...
	GOOS=darwin GOARCH=amd64 go build -o bin/mole -ldflags "$(LDFLAGS)" github.com/thalesfsp/mole
	cd bin && tar c mole | gzip > mole$(version).darwin-amd64.tar.gz && rm mole && cd -
	GOOS=linux GOARCH=amd64 go build -o bin/mole -ldflags "$(LDFLAGS)" github.com/thalesfsp/mole
	cd bin && tar c mole | gzip > mole$(version).linux-amd64.tar.gz && rm mole && cd -
	GOOS=linux GOARCH=arm go build -o bin/mole -ldflags "$(LDFLAGS)" github.com/thalesfsp/mole
	cd bin && tar c mole | gzip > mole$(version).linux-arm.tar.gz && rm mole && cd -
	GOOS=linux GOARCH=arm64 go build -o bin/mole -ldflags "$(LDFLAGS)" github.com/thalesfsp/mole
	cd bin && tar c mole | gzip > mole$(version).linux-arm64.tar.gz && rm mole && cd -
	GOOS=windows GOARCH=amd64 go build -o bin/mole.exe -ldflags "$(LDFLAGS)" github.com/thalesfsp/mole
